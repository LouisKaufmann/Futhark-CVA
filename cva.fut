...
let mc_shortrate (vasicek: Vasicek) (r0:f32) (steps: i32) (delta_t: f32) (rands: [] f32): [steps] f32=
    let xs =
        loop x = [0.05] for i < steps-1 do x ++ [(shortstep vasicek x[i] delta_t rands[i])]
    in xs :> [steps] f32
...
let main (paths:i32) (steps:i32) (swap_term: f32) (payments: i32) (notional:f32) (a:f32) (b:f32) (sigma: f32) (r0: f32)=
    ...
    let vasicek = {a=a, b=b, sigma=sigma}
    ...
    let delta_t = duration/ f32.i32 steps
    ...
    let exposures = map(\x ->
                    ...
                    in exp) mcs
    let avgexp = map(\xs -> reduce (+) 0 xs) (transpose exposures) :> [steps] f32
